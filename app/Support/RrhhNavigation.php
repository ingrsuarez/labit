...
class RrhhNavigation
{
    /**
     * @return array<int, array{key: string, permission: string, title: string, description: string, items: array<int, array<string, mixed>>}>
     */
        ...
        return [
            ...
            [
                ...
                'items' => [
                    ...
                    [
                        'name' => 'Pagos de Haberes',
                        'description' => 'Agrupar liquidaciones en un pago bancario',
                        'route' => route('payroll-payments.index'),
                        'icon' => 'M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z',
                        'permission' => 'payroll-payments.manage',
                    ],
                    [
                        'name' => 'Configuración de Recibos',
                        'description' => 'Firma del empleador',
                        'route' => route('payroll.settings'),
    ...
    /**
     * @return array{title: string, description: string, items: array<int, array<string, mixed>>}|null
     */
    ...
    public static function sectionsForUser(?User $user): array
    {
        ...
        $isPrivileged = $user->hasAnyRole(['admin', 'contador']);

        $result = [];

        foreach (self::sections() as $section) {
            if (! $isPrivileged && ! $user->can($section['permission'])) {
                continue;
            }
    ...
    public static function userCanAccessHub(?User $user): bool
    {
        ...
        return $user->can('personal.section')
            || $user->can('ausencias.section')
            || $user->can('liquidaciones.section');
    }
}
