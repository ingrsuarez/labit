    ...
    <div class="p-4 md:p-6"
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div class="flex items-center gap-3">
                <div>
                    <h1 class="text-2xl font-bold text-gray-800">Facturas de Compra</h1>
                    <p class="text-gray-500 text-sm mt-1">Gestión de facturas de proveedores</p>
                </div>
                @if($total_balance > 0)
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6">
            <div class="flex flex-col md:flex-row gap-3">
                ...
                <div class="w-full md:w-52">
                    <select x-model="supplier_id" class="w-full rounded-lg border-gray-300 text-sm focus:border-zinc-500 focus:ring-zinc-500">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            @foreach($invoices as $invoice)
                                @php
                                    $isPastDue = $invoice->due_date && $invoice->due_date->isPast() && !in_array($invoice->status, ['pagada', 'anulada']);
                                @endphp
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <a href="{{ route('purchase-invoices.show', $invoice) }}" class="text-zinc-700 font-semibold hover:text-zinc-900 text-sm">
                                            {{ $invoice->full_number }}
                                        </a>
                                    </td>
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-700">
                                        ...
                                        @if($invoice->deliveryNotes->isNotEmpty())
                                            <span class="inline-flex items-center gap-1 text-xs text-blue-600 bg-blue-50 border border-blue-200 rounded px-2 py-0.5 ml-2">
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-600 text-right">${{ number_format($invoice->amount_paid, 2, ',', '.') }}</td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                                        <a href="{{ route('purchase-invoices.show', $invoice) }}" class="text-gray-500 hover:text-zinc-700">Ver</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
            ...
            @else
                <div class="p-12 text-center">
                    ...
                    <p class="mt-1 text-sm text-gray-500">Registrá tu primera factura de proveedor.</p>
                </div>
            @endif
        </div>
        </div>
