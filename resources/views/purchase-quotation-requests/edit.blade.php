<x-admin-layout>
    <div class="p-4 md:p-6" x-data="quotationEditForm()">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Editar Solicitud {{ $quotation->number }}</h1>
                <p class="text-gray-500 text-sm mt-1">Modificar solicitud de cotización</p>
            </div>
            <a href="{{ route('purchase-quotation-requests.show', $quotation) }}" class="text-gray-500 hover:text-gray-700 text-sm font-medium">&larr; Volver</a>
        </div>
        ...
        <form method="POST" action="{{ route('purchase-quotation-requests.update', $quotation) }}">
            @csrf
            @method('PUT')
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <div class="md:col-span-3">
                        ...
                        <textarea name="notes" rows="2" class="w-full rounded-lg border-gray-300 text-sm focus:border-zinc-500 focus:ring-zinc-500">{{ old('notes', $quotation->notes) }}</textarea>
                    </div>
                </div>
            </div>

            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
                <h2 class="text-lg font-semibold text-gray-800 mb-4">Insumos</h2>
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-3 py-2 text-center text-xs font-semibold text-gray-500 uppercase w-28">Cantidad</th>
                                <th class="px-3 py-2 text-center text-xs font-semibold text-gray-500 uppercase w-32">Precio Unit.</th>
                                ...
                                <tr>
                                    ...
                                    <td class="px-3 py-2">
                                        <input type="number" :name="'items[' + index + '][unit_price]'" x-model.number="item.unit_price"
                                               min="0" step="0.01"
                                               class="w-28 rounded border-gray-300 text-sm text-right focus:border-zinc-500 focus:ring-zinc-500"
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('purchase-quotation-requests.show', $quotation) }}" class="px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-lg hover:bg-gray-200 text-sm">Cancelar</a>
                <button type="submit" class="px-6 py-3 bg-zinc-700 text-white font-semibold rounded-lg hover:bg-zinc-800 transition-colors text-sm shadow-sm">
                    Guardar Cambios
                </button>
            </div>
        </form>
