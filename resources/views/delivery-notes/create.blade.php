                                        ...
                                        <template x-if="!item.from_po">
                                            <select :name="'items[' + index + '][supply_id_select]'"
                                                ...
                                                @foreach(\App\Models\Supply::active()->orderBy('name')->get() as $sup)
                                                    <option value="{{ $sup->id }}" data-name="{{ $sup->name }}" data-unit="{{ $sup->unit }}">
                                                        {{ $sup->code }} - {{ $sup->name }}
                                                    </option>
                                                @endforeach
                                            </select>
    ...
    <script>
        function deliveryForm() {
            ...
            return {
                ...
                updateSupplyName(index) {
                    const selects = document.querySelectorAll('select[x-model="item.supply_id"]');
                    if (selects[index]) {
                        const opt = selects[index].options[selects[index].selectedIndex];
                        if (opt) {
                            this.items[index].supply_name = opt.dataset.name || '';
                            this.items[index].unit = opt.dataset.unit || '';
                        }
                    }
                }
            }
