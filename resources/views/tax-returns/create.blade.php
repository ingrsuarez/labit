<x-admin-layout>
    <div class="p-4 md:p-6 max-w-4xl" x-data="{
        ...
        taxId: '{{ old('tax_id', $taxes->first()?->id) }}',
        periodYear: {{ old('period_year', (int) date('Y')) }},
        periodMonth: {{ old('period_month', (int) date('n')) }},
        declared: '{{ old('declared_amount', '') }}',
        notes: @js(old('notes', '')),
        loaded: [],
        loading: false,
        needsMonth() {
        ...
        <div class="mb-6 flex justify-between items-start">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Nueva declaración jurada</h1>
        ...
        <form method="POST" action="{{ route('tax-returns.store') }}" class="space-y-6">
            @csrf
