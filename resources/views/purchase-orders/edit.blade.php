<x-admin-layout>
    <div class="p-4 md:p-6" x-data="orderEditForm()">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Editar Orden {{ $order->number }}</h1>
                <p class="text-gray-500 text-sm mt-1">Modificar orden de compra</p>
            </div>
            <a href="{{ route('purchase-orders.show', $order) }}" class="text-gray-500 hover:text-gray-700 text-sm font-medium">&larr; Volver</a>
        </div>
        ...
        <form method="POST" action="{{ route('purchase-orders.update', $order) }}">
            ...
            @if($order->quotation_request_id)
                <input type="hidden" name="quotation_request_id" value="{{ $order->quotation_request_id }}">
            @endif
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="expected_delivery_date" value="{{ old('expected_delivery_date', $order->expected_delivery_date?->format('Y-m-d')) }}"
                               class="w-full rounded-lg border-gray-300 text-sm focus:border-zinc-500 focus:ring-zinc-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">IVA (%) <span class="text-red-500">*</span></label>
                        <input type="number" name="tax_rate" x-model.number="taxRate" value="{{ old('tax_rate', $order->tax_rate) }}" min="0" step="0.01" required
                               class="w-full rounded-lg border-gray-300 text-sm focus:border-zinc-500 focus:ring-zinc-500">
                    </div>
                    <div class="md:col-span-2 lg:col-span-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Notas</label>
                        <textarea name="notes" rows="2" class="w-full rounded-lg border-gray-300 text-sm focus:border-zinc-500 focus:ring-zinc-500">{{ old('notes', $order->notes) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('purchase-orders.show', $order) }}" class="px-6 py-3 bg-gray-100 text-gray-700 font-semibold rounded-lg hover:bg-gray-200 text-sm">Cancelar</a>
