                            ...
                            <div class="flex items-center justify-center gap-2">
                                ...
                                @if($canDelete && !$isChild)
                                    <button type="button" wire:click="removeTest({{ $admissionTest->id }})"
                                            wire:confirm="¿Eliminar esta práctica del protocolo?{{ $hasChildren ? ' (Se eliminarán también las determinaciones hijas)' : '' }}"
                                ...
                                @elseif($canDelete && $isChild && !$hasChildren && !$admissionTest->is_ratified)
                                    <button type="button" wire:click="removeTest({{ $admissionTest->id }})"
                                            wire:confirm="¿Quitar solo esta determinación del protocolo? (El grupo y las demás determinaciones no se eliminan)"
