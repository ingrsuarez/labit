<x-lab-layout>
    <div class="py-6 px-4 md:px-6">
        ...
        {{-- Sección de valores de referencia por especie (solo veterinario) --}}
        @if($test->isVeterinary() && isset($species) && count($species) > 0)
            <div class="mb-6">
                <h3 class="text-lg font-semibold text-amber-700 mb-2 flex items-center gap-2">
                ...
                <p class="text-sm text-gray-500 mb-4">
                    Configurá los rangos de referencia para cada especie. Estos valores se usan automáticamente
                    al crear un protocolo veterinario según la especie del animal.
                </p>

                <div class="bg-white rounded-lg shadow-sm overflow-hidden border border-amber-200">
                    <div class="bg-gradient-to-r from-amber-500 to-amber-600 text-white px-5 py-3">
                        <h4 class="font-semibold">Rangos por Especie</h4>
                    </div>

                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-amber-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-medium text-amber-800 uppercase tracking-wider">Especie</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-amber-800 uppercase tracking-wider">Mín</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-amber-800 uppercase tracking-wider">Máx</th>
                                <th class="px-4 py-3 text-left text-xs font-medium text-amber-800 uppercase tracking-wider">Otros valores ref.</th>
                                <th class="px-4 py-3 text-center text-xs font-medium text-amber-800 uppercase tracking-wider">Acciones</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            @foreach($species as $sp)
                                ...
                                @endphp
                                <tr class="{{ $ref ? 'bg-amber-50/40' : '' }}">
                                    <form action="{{ route('tests.species-references.store', $test) }}" method="POST">
                                        @csrf
                                        <input type="hidden" name="species_id" value="{{ $sp->id }}">
                                        <td class="px-4 py-3">
                                            <span class="font-medium text-gray-900">{{ $sp->name }}</span>
                                            <span class="text-xs text-gray-400 ml-1">({{ $sp->code }})</span>
                                            @if($ref)
                                                <span class="ml-2 inline-block w-2 h-2 rounded-full bg-amber-400" title="Configurado"></span>
                                            @endif
                                        </td>
                                        <td class="px-4 py-3">
                                            <input type="text" name="low" value="{{ $ref->low ?? '' }}"
                                                   class="w-full rounded border-gray-300 text-sm focus:border-amber-500 focus:ring-amber-500"
                                                   placeholder="Ej: 6.5">
                                        </td>
                                        <td class="px-4 py-3">
                                            <input type="text" name="high" value="{{ $ref->high ?? '' }}"
                                                   class="w-full rounded border-gray-300 text-sm focus:border-amber-500 focus:ring-amber-500"
                                                   placeholder="Ej: 8.5">
                                        </td>
                                        <td class="px-4 py-3">
                                            <input type="text" name="other_reference" value="{{ $ref->other_reference ?? '' }}"
                                                   class="w-full rounded border-gray-300 text-sm focus:border-amber-500 focus:ring-amber-500"
                                                   placeholder="Ej: Ausente, Normal">
                                        </td>
                                        <td class="px-4 py-3 text-center whitespace-nowrap">
                                            <div class="flex items-center justify-center gap-2">
                                                <button type="submit"
                                                        class="px-3 py-1.5 bg-amber-100 text-amber-700 rounded hover:bg-amber-200 text-sm font-medium transition-colors">
                                                    Guardar
                                                </button>
                                                @if($ref)
                                                    <button type="button"
                                                            onclick="if(confirm('¿Eliminar referencia de {{ $sp->name }}?')) document.getElementById('del-sp-{{ $sp->id }}').submit()"
                                                            class="px-3 py-1.5 bg-red-100 text-red-700 rounded hover:bg-red-200 text-sm font-medium transition-colors">
                                                        Eliminar
                                                    </button>
                                                @endif
                                            </div>
                                ...
                                @if($ref)
                                    <form id="del-sp-{{ $sp->id }}" action="{{ route('tests.species-references.destroy', [$test, $ref]) }}" method="POST" class="hidden">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                @endif
                ...
                @php
                    $configuredCount = $test->speciesReferences->count();
                @endphp
                @if($configuredCount > 0)
                    <div class="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                        <div class="flex items-center text-amber-700">
                            ...
                            <span class="text-sm font-medium">{{ $configuredCount }} especie{{ $configuredCount > 1 ? 's' : '' }} configurada{{ $configuredCount > 1 ? 's' : '' }}</span>
                        </div>
                    </div>
                @endif
                ...
                <div class="mt-4 bg-blue-50 rounded-lg p-4 border border-blue-200">
                    ...
                    <ul class="text-xs text-blue-700 space-y-1">
                        <li>• Completá <strong>Mín</strong> y <strong>Máx</strong> para rangos numéricos (ej: 6.5 - 8.5).</li>
                        <li>• Usá <strong>Otros valores ref.</strong> para textos libres (ej: "Negativo", "< 10 UFC/ml").</li>
                        <li>• Al crear un protocolo veterinario, el sistema toma automáticamente el valor según la especie del animal.</li>
                    </ul>
                </div>
            </div>
        @else
        ...
        @endif {{-- cierra @else de isVeterinary --}}

        @endif {{-- cierra @if($isParent) --}}
    </div>
</x-lab-layout>
