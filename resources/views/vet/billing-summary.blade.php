<x-lab-layout title="Resumen por período — Veterinario">
    <div class="py-6 px-4 md:px-6 lg:px-8 mt-14 md:mt-0">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form action="{{ route('vet.billing-summary') }}" method="GET" class="flex flex-wrap gap-4 items-end">
                <div class="w-full sm:w-64">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Cliente (veterinaria) *</label>
                    <select name="customer_id" required class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-amber-500 focus:border-amber-500">
                ...
                <div class="w-44">
                    ...
                    <select name="format" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-amber-500 focus:border-amber-500">
                ...
                <div class="w-40">
                    ...
                    <input type="date" name="date_from" value="{{ $dateFrom }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-amber-500 focus:border-amber-500">
                </div>
                <div class="w-40">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Hasta</label>
                    <input type="date" name="date_to" value="{{ $dateTo }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-amber-500 focus:border-amber-500">
                </div>
        ...
        @if($rows !== null)
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border p-4">
                    ...
                    <p class="text-2xl font-bold text-amber-600">${{ number_format($totals['total_amount'], 2, ',', '.') }}</p>
                </div>
            </div>
            ...
            <div class="mb-4 flex flex-wrap justify-end gap-2">
                <a href="{{ route('vet.billing-summary.exportPdf') }}?{{ $exportQuery }}" class="inline-flex items-center px-4 py-2 bg-gray-700 text-white rounded-lg hover:bg-gray-800 text-sm">
                    <i class="bi bi-file-earmark-pdf mr-2"></i> Exportar PDF
                </a>
                <a href="{{ route('vet.billing-summary.exportExcel') }}?{{ $exportQuery }}" class="inline-flex items-center px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm">
                            ...
                            <tfoot class="bg-amber-50">
                                <tr>
                                    ...
                                    @else
                                        ...
                                        <td class="px-4 py-4 text-right text-lg font-bold text-amber-600">${{ number_format($totals['total_amount'], 2, ',', '.') }}</td>
                                    @endif
                                </tr>
                            </tfoot>
                        </table>
