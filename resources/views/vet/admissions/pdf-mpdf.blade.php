...
<head>
    <meta charset="UTF-8">
    <title>Protocolo {{ $vetAdmission->protocol_number }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
        ...
        <table width="100%" style="border-bottom: 2px solid #030303; padding-bottom: 10px;">
            <tr>
                ...
                <td width="70%" style="vertical-align: middle; text-align: right;">
                    <span style="font-size: 20px; font-weight: bold; color: #C3C3C3;">IPAC Laboratorio Veterinario</span>
                </td>
            </tr>
        </table>
    </htmlpageheader>
    ...
    <htmlpagefooter name="myFooter">
        <div style="font-size: 8px; color: #999; text-align: center; border-top: 1px solid #eee; padding-top: 5px;">
            Protocolo {{ $vetAdmission->protocol_number }} | Página {PAGENO} de {nbpg} | Generado: {{ now()->format('d/m/Y H:i') }}
        </div>
    </htmlpagefooter>
    <sethtmlpagefooter name="myFooter" value="on" />

    @php
        $validatedTests = $vetAdmission->vetTests->where('is_validated', true);
        $allProtocolTestIds = $vetAdmission->vetTests->pluck('test_id')->toArray();
    ...
    <table class="det-table" cellpadding="0" cellspacing="0">
        ...
        @foreach($orderedTests as $entry)
            ...
            @if($isParent && !$isSub)
                ...
                @if($vt->test?->method)
                    <tr class="det-method">
                        <td colspan="4">{{ $vt->test->method }}</td>
                    </tr>
                @endif
            @elseif($isSub)
                <tr class="det-subparent">
                    <td colspan="4" style="padding-left: {{ $indent }}px;">
                        {{ strtoupper($vt->test->name ?? 'N/A') }}
                    </td>
                </tr>
                @if($vt->test?->method)
                    <tr class="det-method">
                        <td colspan="4" style="padding-left: {{ $indent }}px;">{{ $vt->test->method }}</td>
                    </tr>
                @endif
            @elseif($isChild)
                <tr class="det-child">
        ...
        <table width="100%">
            <tr>
                <td width="50%" style="vertical-align: bottom;">
                    ...
                    <div class="validation-text" style="font-size: 9px; color: #666; margin-top: 4px;">Los resultados corresponden a la muestra recibida</div>
                </td>
