                            ...
                            @foreach($vetAdmission->getVetTestsOrderedForDisplay() as $idx => $entry)
                                ...
                                <tr class="{{ $isTreeChild ? 'bg-gray-50/50' : '' }} {{ $vt->is_validated ? 'bg-green-50/40' : '' }}">
                                    ...
                                    <td class="px-4 py-2 text-center">
                                        ...
                                        @if($vetCanRemoveRow)
                                            <button type="button"
                                                    onclick="if(confirm('{{ $isTreeChild ? "¿Quitar solo esta determinación del protocolo? (El grupo y las demás no se eliminan)" : "¿Quitar esta práctica del protocolo?" }}')) vetDeleteAction('{{ route('vet.admissions.removeTest', [$vetAdmission, $vt]) }}')"
                        ...
                        <div x-show="zebraAvailable && labelRows.length > 0" class="space-y-4 border-t border-gray-100 pt-4">
                            ...
                            <div class="bg-gray-50 rounded-lg p-3 border border-dashed border-gray-300">
                                ...
                                <div class="bg-white border border-gray-200 rounded p-2 text-center font-mono text-xs leading-relaxed" x-show="firstPreviewRow()">
                                    ...
                                    <div>{{ Str::limit($vetAdmission->animal_name ?? $vetAdmission->owner_name ?? 'N/A', 30) }}</div>
                                    <div class="text-gray-500">
                                        <span x-text="previewField('branch_name', 'VETERINARIO')"></span> |
                                        <strong x-text="previewField('material', '?')"></strong> |
                                        <span x-text="previewField('entry_date', '{{ $vetAdmission->date?->format('d/m/Y') ?? '' }}')"></span>
                                    </div>
                                </div>
                            </div>
