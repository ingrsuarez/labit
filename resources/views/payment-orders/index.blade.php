    ...
    <div class="p-4 md:p-6"
         x-data="{
             ...
             fetchResults() {
                 ...
                 const url = '{{ route('payment-orders.index') }}' + (params.toString() ? '?' + params.toString() : '');
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Órdenes de Pago</h1>
                <p class="text-gray-500 text-sm mt-1">Gestión de pagos a proveedores</p>
            </div>
            <a href="{{ route('payment-orders.create') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            @if($paymentOrders->count() > 0)
                            ...
                            @foreach($paymentOrders as $order)
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <a href="{{ route('payment-orders.show', $order) }}" class="text-zinc-700 font-semibold hover:text-zinc-900 text-sm">
                                            {{ $order->number }}
                                        </a>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                                        <a href="{{ route('payment-orders.show', $order) }}" class="text-gray-500 hover:text-zinc-700">Ver</a>
                                    </td>
                                </tr>
                            @endforeach
                        </tbody>
                ...
                @if($paymentOrders->hasPages())
                    <div class="px-4 py-3 border-t border-gray-200">{{ $paymentOrders->links() }}</div>
                @endif
            @else
                <div class="p-12 text-center">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No hay órdenes de pago</h3>
                    <p class="mt-1 text-sm text-gray-500">Creá tu primera orden de pago.</p>
                </div>
            @endif
        </div>
        </div>
