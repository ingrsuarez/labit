<x-admin-layout>
    <div class="p-4 md:p-6" x-data="paymentEditForm()">
        ...
        <form method="POST" action="{{ route('payment-orders.update', $paymentOrder) }}">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5" x-show="paymentMode === 'portfolio_echeq'" x-cloak>
                ...
                <p class="text-sm text-gray-500 mb-4">La suma de los e-cheqs seleccionados debe igualar el total a pagar.</p>
    ...
    <script>
        function paymentEditForm() {
            ...
            const initialPortfolioIds = @json($initialPortfolioIds);
            const defaultMode = '{{ old('payment_mode', $paymentOrder->portfolioEcheqPayments->isNotEmpty() ? 'portfolio_echeq' : 'legacy') }}';

            return {
                supplierId: '{{ old('supplier_id', $paymentOrder->supplier_id) }}',
                paymentMode: defaultMode,
                portfolioRows,
                selectedPortfolioIds: initialPortfolioIds.map(String),
                invoices: invoices,
                onSupplierChange() {
