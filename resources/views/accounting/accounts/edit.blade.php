...
<div class="p-4 md:p-6">
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-gray-800">Editar Cuenta Contable</h1>
        <p class="text-gray-500 text-sm mt-1">{{ $account->code }} — {{ $account->name }}</p>
    </div>
    ...
    <div class="max-w-2xl bg-white rounded-xl border border-gray-200 shadow-sm p-6">
        <form action="{{ route('accounting.accounts.update', $account) }}" method="POST"
              x-data="{
                  ...
                  parentType: '{{ $account->parent ? $account->parent->type : '' }}',
                  parentLevel: {{ $account->parent ? $account->parent->level + 1 : 0 }},
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="type" id="type" :disabled="parentId != ''" required
                            ...
                            <option value="{{ $key }}" :selected="parentId ? parentType === '{{ $key }}' : '{{ old('type', $account->type) }}' === '{{ $key }}'">{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input type="number" name="level" id="level" min="1" max="4" required
                               :value="parentId ? parentLevel : '{{ old('level', $account->level) }}'"
                               :readonly="parentId != ''"
                ...
                <div class="flex items-center gap-2">
                    <input type="checkbox" name="is_header" id="is_header" value="1" {{ old('is_header', $account->is_header) ? 'checked' : '' }}
