...
if (old('lines') && is_array(old('lines'))) {
    ...
    $initialLines = $journal->lines->map(fn ($l) => [
        'account_id' => (string) $l->accounting_account_id,
        'description' => (string) ($l->description ?? ''),
        'debit' => (float) $l->debit,
...
<div class="p-4 md:p-6 max-w-5xl">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
        <div>
            <h1 class="text-2xl font-bold text-gray-800">Editar asiento manual</h1>
            <p class="text-gray-500 text-sm mt-1">{{ $journal->date->format('d/m/Y') }} · Nº {{ $journal->date->format('Y') }}-{{ str_pad((string) $journal->number, 4, '0', STR_PAD_LEFT) }}</p>
        </div>
    ...
    <form method="POST" action="{{ route('accounting.journal.update', $journal) }}" class="space-y-6" x-data="journalForm(@js($initialLines))">
        @csrf
        @method('PUT')
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm p-4 md:p-6 space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="date" name="date" id="date" required value="{{ old('date', $journal->date->toDateString()) }}"
                ...
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="description" id="description" required maxlength="500" value="{{ old('description', $journal->description) }}"
                           class="w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-zinc-500 focus:ring-zinc-500">
                </div>
            </div>
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div class="text-sm flex items-start gap-2" :class="isBalanced ? 'text-green-600' : 'text-red-600'">
                    <svg x-show="isBalanced" class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                    <svg x-show="!isBalanced" x-cloak class="w-5 h-5 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
