<x-lab-layout title="Santa Cruz — sincronización FTP">
    <div class="py-6 px-4 md:px-6 lg:px-8 mt-14 md:mt-0">
        {{-- Overlay FTP sin Alpine: en producción, si Vite/Alpine falla, el resto de la página sigue visible. --}}
        <div id="santa-cruz-ftp-busy"
             class="hidden fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-6"
             aria-hidden="true">
            <div class="max-w-md rounded-xl bg-white p-6 shadow-xl text-center space-y-3">
                <p class="text-lg font-semibold text-gray-900">Sincronización en curso…</p>
        ...
        <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-wrap gap-2">
                <form action="{{ route('lab.santa-cruz.sync.scan') }}" method="POST"
                      onsubmit="(function(){var e=document.getElementById('santa-cruz-ftp-busy');if(e){e.classList.remove('hidden');e.setAttribute('aria-hidden','false');}})();">
                    @csrf
        ...
        @else
            <div x-data="santaCruzSync({
                ...
                <form action="{{ route('lab.santa-cruz.sync.import') }}" method="POST" id="import-form" class="space-y-4"
                      onsubmit="if (! confirm('¿Importar los archivos seleccionados? Se crearán admisiones y se moverán los XML a la carpeta procesados en el FTP.')) { return false; } (function(){var e=document.getElementById('santa-cruz-ftp-busy');if(e){e.classList.remove('hidden');e.setAttribute('aria-hidden','false');}})();">
                    @csrf
