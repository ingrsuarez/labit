        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <form method="POST" action="{{ route('api-clients.update', $client) }}" class="px-6 py-6 space-y-5">
                ...
                <div>
                    ...
                    <select name="patient_data_level" id="patient_data_level" required
                            class="w-full border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        @foreach(\App\Models\ApiClient::PATIENT_DATA_LEVELS as $value => $label)
                            <option value="{{ $value }}"
                                @selected(old('patient_data_level', $client->patient_data_level) === $value)>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                    <p class="text-xs text-gray-500 mt-1">
                        ...
                        Cambiar a <strong>Estándar</strong> requiere justificación legal porque expone datos
                        personales sensibles a la integración.
                    </p>
