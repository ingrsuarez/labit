<x-admin-layout>
    <div class="p-4 md:p-6 max-w-2xl">
        <div class="mb-6 flex justify-between items-start">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Nuevo impuesto</h1>
                <p class="text-gray-500 text-sm mt-1">Cuenta de pasivo donde impactará la DDJJ confirmada</p>
            </div>
        ...
        <form method="POST" action="{{ route('taxes.store') }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Nombre <span class="text-red-500">*</span></label>
                <input type="text" name="name" value="{{ old('name') }}" required maxlength="255" class="w-full rounded-lg border-gray-300 text-sm focus:border-indigo-500 focus:ring-indigo-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Jurisdicción</label>
                <input type="text" name="jurisdiction" value="{{ old('jurisdiction') }}" class="w-full rounded-lg border-gray-300 text-sm">
            </div>
            <div>
                ...
                <select name="liability_account_id" required class="w-full rounded-lg border-gray-300 text-sm">
                    ...
                    @foreach($liabilityAccounts as $acc)
                        <option value="{{ $acc->id }}" @selected(old('liability_account_id') == $acc->id)>{{ $acc->code }} — {{ $acc->name }}</option>
                    @endforeach
                </select>
            </div>
            <div>
                ...
                <select name="frequency" class="w-full rounded-lg border-gray-300 text-sm">
                    ...
                    <option value="annual" @selected(old('frequency') === 'annual')>Anual</option>
                </select>
            </div>
            <div class="flex items-center gap-2">
                <input type="checkbox" name="is_active" id="is_active" value="1" class="rounded border-gray-300 text-indigo-600" checked>
