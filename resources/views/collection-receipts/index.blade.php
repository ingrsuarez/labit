    ...
    <div class="p-4 md:p-6"
         x-data="{
             ...
             fetchResults() {
                 ...
                 const url = '{{ route('collection-receipts.index') }}' + (params.toString() ? '?' + params.toString() : '');
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Recibos de Cobro</h1>
                <p class="text-gray-500 text-sm mt-1">Gestión de cobros a clientes</p>
            </div>
            <a href="{{ route('collection-receipts.create') }}"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            @if($collectionReceipts->count() > 0)
                    ...
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Método de Pago</th>
                            ...
                            @foreach($collectionReceipts as $receipt)
                                ...
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 whitespace-nowrap">
                                        <a href="{{ route('collection-receipts.show', $receipt) }}" class="text-zinc-700 font-semibold hover:text-zinc-900 text-sm">
                                            {{ $receipt->number }}
                                        </a>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 whitespace-nowrap text-right text-sm">
                                        <div class="inline-flex flex-wrap items-center justify-end gap-2">
                                            <a href="{{ route('collection-receipts.pdf', $receipt) }}"
                                               title="Descargar PDF"
                                               class="inline-flex items-center px-2.5 py-1 bg-red-600 text-white text-xs font-medium rounded-md hover:bg-red-700 transition-colors">
                                            ...
                                            <a href="{{ route('collection-receipts.show', $receipt) }}" class="text-gray-500 hover:text-zinc-700">Ver</a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                ...
                @if($collectionReceipts->hasPages())
                    <div class="px-4 py-3 border-t border-gray-200">{{ $collectionReceipts->links() }}</div>
                @endif
            @else
                <div class="p-12 text-center">
                    ...
                    <h3 class="mt-2 text-sm font-medium text-gray-900">No hay recibos de cobro</h3>
                    <p class="mt-1 text-sm text-gray-500">Creá tu primer recibo de cobro.</p>
                </div>
            @endif
        </div>
        </div>
