<x-lab-layout title="Editar Presupuesto">
    <div class="p-4 md:p-6" x-data="quoteEditForm()">
        <!-- Header -->
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Editar Presupuesto</h1>
                <p class="text-gray-500 text-sm mt-1">{{ $quote->quote_number }}</p>
            </div>
            <a href="{{ route('quotes.show', $quote) }}" class="text-gray-500 hover:text-gray-700 text-sm font-medium">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5">
                    ...
                    <div class="space-y-4">
                        <div>
                            ...
                            <input type="date" name="valid_until" value="{{ $quote->valid_until?->format('Y-m-d') }}"
                                   class="w-full rounded-lg border-gray-300 text-sm focus:border-teal-500 focus:ring-teal-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Notas / Observaciones</label>
                            <textarea name="notes" rows="3"
                                      class="w-full rounded-lg border-gray-300 text-sm focus:border-teal-500 focus:ring-teal-500">{{ $quote->notes }}</textarea>
                        </div>
                    </div>
                </div>
    ...
    <script>
        function quoteEditForm() {
            return {
                ...
                customerName: @json($quote->customer_name),
                customerEmail: @json($quote->customer_email ?? ''),
                customerSearch: '',
                customerResults: [],
                searchQuery: '',
                searchResults: [],
                ...
                addManualItem() {
                    this.items.push({ test_id: null, description: '', quantity: 1, unit_price: 0, children: [] });
                },
