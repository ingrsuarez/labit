<x-admin-layout>
    @php
        $isElectronic = $invoice->pointOfSale && $invoice->pointOfSale->is_electronic;
    @endphp
    <div class="p-4 md:p-6" x-data="creditNoteForm()">
        <div class="flex items-center justify-between mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">Nueva Nota de Crédito</h1>
                <p class="text-gray-500 text-sm mt-1">Generar nota de crédito desde factura</p>
            </div>
        ...
        <div class="mb-5 p-4 bg-blue-50 border border-blue-200 rounded-lg">
            <div class="flex items-center gap-2">
                ...
                <p class="text-sm text-blue-800">
                    Generando nota de crédito a partir de Factura
                    <a href="{{ route('sales-invoices.show', $invoice) }}" class="font-semibold underline hover:text-blue-900">{{ $invoice->full_number }}</a>
                </p>
            </div>
        </div>
        ...
        @if($isElectronic)
            <div class="mb-5 p-4 bg-indigo-50 border border-indigo-200 rounded-lg">
                <div class="flex items-center gap-2">
                    ...
                    <p class="text-sm text-indigo-800 font-medium">Nota de Crédito Electrónica — Será autorizada automáticamente en AFIP</p>
                </div>
            </div>
        @endif
        ...
        <form method="POST" action="{{ route('credit-notes.store') }}">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    <div>
                        ...
                        <input type="text" readonly value="NC {{ $invoice->voucher_type }}"
                               class="w-full rounded-lg border-gray-200 bg-gray-100 text-gray-600 text-sm cursor-not-allowed">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Punto de Venta</label>
                        <input type="text" readonly
                               value="{{ $invoice->pointOfSale ? $invoice->pointOfSale->code . ' - ' . $invoice->pointOfSale->name : '-' }}"
                               class="w-full rounded-lg border-gray-200 bg-gray-100 text-gray-600 text-sm cursor-not-allowed">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Cliente</label>
                        <input type="text" readonly value="{{ $invoice->customer->name }}"
                               class="w-full rounded-lg border-gray-200 bg-gray-100 text-gray-600 text-sm cursor-not-allowed">
                    </div>
                    <div class="md:col-span-2 lg:col-span-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Motivo <span class="text-red-500">*</span></label>
                        <textarea name="reason" rows="3" required placeholder="Motivo de la nota de crédito..."
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-5 mb-5">
                ...
                <div x-show="items.length === 0" class="text-center py-8 text-gray-400 text-sm">
                    No hay ítems cargados desde la factura
                </div>
            </div>
            ...
            return {
                items: [
                    @foreach($invoice->items as $item)
                    {
                        ...
                        iva_rate: {{ floatval($item->iva_rate) }},
                    },
                    @endforeach
                ],
