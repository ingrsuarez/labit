<x-admin-layout>
    <div class="p-4 md:p-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
            ...
            <div class="flex items-center gap-3 mt-3 md:mt-0">
                @if($creditNote->status === 'confirmada')
                    <a href="{{ route('credit-notes.pdf', $creditNote) }}"
        ...
        @elseif(!$creditNote->is_electronic)
            <div class="mb-5 p-4 bg-gray-50 border border-gray-200 rounded-lg">
                <div class="flex items-center gap-2">
                    <svg class="w-5 h-5 text-gray-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                    </svg>
                    <p class="text-sm text-gray-600">Nota de crédito manual — sin factura asociada en el sistema</p>
                </div>
            </div>
        @endif
        ...
        @if($creditNote->is_electronic)
            ...
            @else
                <div class="bg-white rounded-xl shadow-sm border-2 border-red-200 p-5 mb-6">
                    ...
                    @if($creditNote->afip_response)
                        @if(isset($creditNote->afip_response['error']))
                            <p class="text-sm text-red-700 mt-2">Error: {{ $creditNote->afip_response['error'] }}</p>
                        @endif
                        @php
                            $obs = $creditNote->afip_response['FeDetResp']['FECAEDetResponse']['Observaciones']['Obs'] ?? null;
