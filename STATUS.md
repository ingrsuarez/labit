...
> Última actualización: 2026-05-15 (**master**: **v1.98.3** hotfix presupuestos + **v1.99.0** deducciones RRHH en código; **develop**: alineado; **release** previo **v1.98.1** planilla pendientes / **v1.97.0** Santa Cruz FTP)
...
| **Última completada (cola agente)** | **v1.98.3** — Índice presupuestos (`QuoteIndexPaginationTest`); **v1.99.0** — Deducciones RRHH (`DeductionBaseResolver`, `total_haberes`) |
| **Hotfix aplicado** | **v1.98.3** (2026-05-15): listado `/quotes`. **v1.98.2** (2026-05-14): ingesta API sin dedup por mensaje HL7; idempotencia por `batch_id`. Además: vet doble submit, Eliminar protocolo; **sesión por inactividad** (`SESSION_IDLE_TIMEOUT_MINUTES`, `users.last_activity_at`, middleware `enforce.idle`) |
